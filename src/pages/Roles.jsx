...
export default function Roles() {
  ...
  return (
    <div style={{paddingTop:'72px',background:'var(--cream)',minHeight:'100vh'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))',gap:'1.5rem',maxWidth:'1000px',margin:'3rem auto',padding:'0 2rem'}}>
        {[
          ['Customer','/images/customer.png','Discover and buy authentic tribal crafts. Support artisan communities directly.'],
          ['Artisan','/images/artisan.png','List your handcrafted pieces. Reach buyers worldwide. Get fair prices for your craft.'],
          ['Consultant','https://images.unsplash.com/photo-1552664730-d307ca884978?w=500&q=80','Guide artisans. Help them grow their businesses and reach new markets.'],
        ].map(([role,imgUrl,desc]) => (
          <div key={role} onClick={() => set('role', role.toLowerCase())} style={{overflow:'hidden',textAlign:'center',border:'2px solid',borderColor:form.role===role.toLowerCase()?'var(--ochre)':'var(--sand)',borderRadius:'8px',cursor:'pointer',background:form.role===role.toLowerCase()?'rgba(196,116,42,0.05)':'#fff',transition:'all 0.2s',display:'flex',flexDirection:'column'}}>
            <div style={{height:'140px',width:'100%',backgroundImage:`url(${imgUrl})`,backgroundSize:'cover',backgroundPosition:'center'}}></div>
            <div style={{padding:'1.5rem'}}>
              <h3 style={{color:'var(--deep-brown)',marginBottom:'0.5rem'}}>{role}</h3>
              <p style={{color:'var(--mud)',fontSize:'0.9rem',lineHeight:1.5,margin:0}}>{desc}</p>
            </div>
          </div>
        ))}
      </div>
